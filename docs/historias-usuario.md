...
| ID | Clave | Tarea | Atributo | Responsable |
| --- | --- | --- | --- | --- |
| TEC-01 | `SOL-50` | Versionar las APIs públicas con compatibilidad hacia atrás. | Integración / Modificación | Alejandro Forero |
| TEC-02 | `SOL-51` | Definir e instrumentar el presupuesto de latencia por dependencia (≤ 120 ms; timeout 700 ms). | Latencia | David Rodríguez |
| TEC-03 | `SOL-52` | Implementar observabilidad extremo a extremo (trazas, métricas p95/p99, logs sin PII). | Todos | Juan S. Sánchez |
| TEC-04 | `SOL-53` | Implementar autoescalado y contrapresión (500 → 50.000 cotizaciones/min en ≤ 60 s). | Escalabilidad | Yesid Marín |
| TEC-05 | `SOL-54` | Diseñar la continuidad multi-zona (RTO ≤ 10 min, RPO ≤ 30 s) y multi-región (RTO ≤ 5 min). | Disponibilidad | Yesid Marín |
| TEC-06 | `SOL-55` | Aislar cada integración externa tras un adaptador con contrato estable. | Integración / Modificación | David Rodríguez |
| TEC-07 | `SOL-56` | Implementar cifrado, tokenización de PII, mínimo privilegio y controles PCI-DSS. | Seguridad | David Rodríguez |
| TEC-08 | `SOL-57` | Establecer el pipeline de CI/CD con despliegues sin downtime. | Disponibilidad / Modificación | Juan S. Sánchez |
| TEC-09 | `SOL-58` | Construir el banco de experimentos de arquitectura reproducible. | Todos | Alejandro Forero |
| TEC-10 | `SOL-59` | Detectar y bloquear patrones anómalos de fraude o ataque en ≤ 1 s. | Seguridad / Disponibilidad | David Rodríguez |
...
Todas las historias se acotaron para cumplir el criterio *Pequeña* del curso: **ninguna requiere
más de dos días de una sola persona**.

### Reparto por integrante

| Integrante | Historias | Tareas | **Total** |
| --- | --: | --: | --: |
| Juan Alejandro Forero Gómez | 9 | 3 | **12** |
| David Armando Rodríguez Varón | 8 | 4 | **12** |
| Juan Sebastián Sánchez Tabares | 9 | 2 | **11** |
| Yesid Marín | 9 | 2 | **11** |
...
| Historia | Se solapaba con | Corrección |
| --- | --- | --- |
| HU-005 | HU-006 (cobro), HU-007 (firma) | Acotada a suscripción + emisión |
| HU-003 | HU-017 (auditoría) | Retirado el criterio de reconstrucción |
| HU-012 | TEC-04 (contrapresión) | Retirado el criterio de 1.000.000 de eventos |
| HU-014 | Infraestructura | Separado el aislamiento de carga por socio |

### 3. Cinco historias no cabían en dos días · `[Pequeña]`

| Original | Resultado |
| --- | --- |
| HU-015 | HU-015 Editar regla en consola + **HU-033** Publicar versión sin redespliegue |
| HU-021 | HU-021 Consultar billetera offline + **HU-034** Proteger la caché local |
| HU-022 | HU-022 Sincronizar al recuperar red + **HU-035** Resolver conflictos |
| HU-025 | HU-025 Reportar con evidencia + **HU-036** Reanudar carga sin duplicar |
| HU-019 | Acotada a documento y prueba de vida; biometría → HU-020, cifrado → TEC-07 |
